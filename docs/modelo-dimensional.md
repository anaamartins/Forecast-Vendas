...
A Bus Matrix representa a relação entre os processos de negócio e as dimensões utilizadas no modelo.

| Dimensão | Desempenho comercial | Fatores macroeconómicos |
|---|:---:|:---:|
| `DimCliente` | ✓ |  |
| `DimEmpresa` | ✓ |  |
| `DimPais` | ✓ | ✓ |
...
O modelo utiliza um esquema de **fact constellation**, uma vez que possui duas tabelas de factos que partilham dimensões conformadas.

As dimensões partilhadas são:
...
A utilização de dimensões conformadas permite analisar os diferentes processos de negócio através das mesmas perspetivas geográficas e temporais.

## Diagrama do modelo

![Modelo dimensional](../assets/images/modelo-dimensional.png)

*Modelo dimensional com as tabelas de factos, dimensões e respetivas relações.*
